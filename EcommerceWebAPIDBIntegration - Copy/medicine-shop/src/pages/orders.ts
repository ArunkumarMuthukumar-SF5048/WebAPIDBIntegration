...
export async function renderOrders(container: HTMLElement) {
  ...
  async function createTable() {
    ...
    orders.forEach((order) => {
      ...
      if (order.userID == currentUser!.userID) {
        ...
        row.innerHTML = `<td>${order.orderID}</td> <td>${order.userID}</td> <td>${order.productID}</td> <td>${order.productCount}</td> <td>${order.totalPrice}</td> 
  ...
  async function cancelOrder(orderId: number) {
    ...
    await APICALLS.cancelOrder(currentUser!.userID, orderId);
