import { User, ProductInfo, Orders } from '../models/model';
let url = "http://localhost:5082/api";
...
export async function fetchProduct(): Promise<ProductInfo[]> {
    let apiURL = `${url}/productInfocontroller/products`;
    let response = await fetch(apiURL);
    if (!response.ok) {
        throw new Error("Fail to fetch data");
    }
...
export async function getIndividualProduct(productID: number): Promise<ProductInfo | null> {
    let apiURL = `${url}/productInfocontroller/get/products/${productID}`;
    let response = await fetch(apiURL);
    if (!response.ok) {
        return null;
    }
...
export async function addNewProduct(product: ProductInfo): Promise<string> {
    let apiURL = `${url}/productInfocontroller/add/newProduct`;
    let response = await fetch(apiURL, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
...
export async function checkProductExist(productName: string): Promise<boolean> {
    let apiURL = `${url}/productInfocontroller/products/${productName}`;
    let response = await fetch(apiURL);
    if (!response.ok) {
        throw new Error("Fail to fetch data");
    }
...
export async function editProductDetail(product: ProductInfo): Promise<void> {
    let apiURL = `${url}/productInfocontroller/new/product/edit`;
    let response = await fetch(apiURL, {
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json'
...
export async function deleteProductDetail(productID: number): Promise<void> {
    ...
    const response = await fetch(`${url}/productInfocontroller/delete/${productID}`, {
        method: 'DELETE'
    });
    if (!response.ok) {
...
export async function getIndividualOrder(orderID: number): Promise<Orders | null> {
...
export async function cancelOrder(userID: number, orderID: number): Promise<void> {
    let apiURL = `${url}/orderscontroller/cancel/${userID}/${orderID}`;
    let response = await fetch(apiURL, {
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json'
...
export async function addNewOrder(userID: number, productID: number, quantity: number): Promise<void> {
    let apiURL = `${url}/orderscontroller/add/newOrder/${userID}/${productID}/${quantity}`;
    let response = await fetch(apiURL, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
